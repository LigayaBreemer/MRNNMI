##### PILOT STUDY ##############################################################
# To determine the appropriate number of imputations, the performance of the 
# three MI procedures with correctly specified models (DRNN11, MICE1, and 
# MRNN11) will be compared for 5, 10, and 20 imputations.
...
p2 <- exp(.3*x1 - 0.7*x2 + 1.2*x3)/ (1 + exp(.3*x1 - 0.7*x2 + 1.2*x3) + 
                                       exp(.2*x1 + 0.6*x2 - 1.5*x3))
p3 <- exp(.2*x1 + 0.6*x2 - 1.5*x3)/ (1 + exp(.3*x1 - 0.7*x2 + 1.2*x3) + 
                                       exp(.2*x1 + 0.6*x2 - 1.5*x3))
p1 <- 1 - p2 - p3
...
MRNNI <- function(iterations, samp){
  ...
  # for the nonrespondents in the original sample and standardize these using 
  # the parameters obtained in steps 2 and 3.
  ...
  distances <- apply(pred.to_be_imputed, 1, function(x) apply(pred.boot_obs, 1, 
                                                              distance, x))
...
DRNNI <- function(iterations, samp){
  ...
  pred1 <- fitted(mod1)[,-1]
  # We are manually standardizing because we need to save the means and sds for
  # later.
  mu1 <- colMeans(pred1)
  ...
  # step 2b: Using the bootstrap sample, fit a logistic regression model on the 
  # response indicators using the auxiliary data and save the standardized 
  # predicted values.
  ...
  # Here, each column contains the distances from a subject i with missing Y in
  # the original data set to all subjects j with observed Y in the bootstrap 
  # sample.
  distances <- apply(pred.to_be_imputed, 1, function(x) apply(pred.boot_obs, 1, 
                                                              distance, x))
...
# the SEs. Each row contains the lower and upper bound for one outcome category.
...
within_MRNNI5 <- CIs_MRNNI5[,1]<=original_proportions & 
  original_proportions<=CIs_MRNNI5[,2]
within_MRNNI10 <- CIs_MRNNI10[,1]<=original_proportions & 
  original_proportions<=CIs_MRNNI10[,2]
within_MRNNI20 <- CIs_MRNNI20[,1]<=original_proportions & 
  original_proportions<=CIs_MRNNI20[,2]
...
within_DRNNI5 <- CIs_DRNNI5[,1]<=original_proportions & 
  original_proportions<=CIs_DRNNI5[,2]
within_DRNNI10 <- CIs_DRNNI10[,1]<=original_proportions & 
  original_proportions<=CIs_DRNNI10[,2]
within_DRNNI20 <- CIs_DRNNI20[,1]<=original_proportions & 
  original_proportions<=CIs_DRNNI20[,2]
...
# Then check if the original proportions are within the CIs (TRUE/FALSE).
within_MICE5 <- CIs_MICE5[,1]<=original_proportions & 
  original_proportions<=CIs_MICE5[,2]
within_MICE10 <- CIs_MICE10[,1]<=original_proportions & 
  original_proportions<=CIs_MICE10[,2]
within_MICE20 <- CIs_MICE20[,1]<=original_proportions & 
  original_proportions<=CIs_MICE20[,2]
...
# Because I am running this over different sessions, I need to set a seed for 
# every session
set.seed(22022022)
...
# Generate dependent variable:
# Generate outcome probabilities for each category
p2 <- exp(.1*x2^2 - .2*x1*x3 + 0.3*x1 - x2 + 0.9*x3)/ 
  (1 + exp(.1*x2^2 - .2*x1*x3 + 0.3*x1 - x2 + 0.9*x3) + 
     exp(0.2*x2^2 + .3*x1*x3 - 0.4*x1 - 0.8*x2 - 1.1*x3))
p3 <- exp(0.2*x2^2 + .3*x1*x3 - 0.4*x1 - 0.8*x2 - 1.1*x3)/ 
  (1 + exp(.1*x2^2 - .2*x1*x3 + 0.3*x1 - x2 + 0.9*x3) + 
     exp(0.2*x2^2 + .3*x1*x3 - 0.4*x1 - 0.8*x2 - 1.1*x3))
p1 <- 1 - p2 - p3
probs <- cbind(p1, p2, p3)
...
# Generate dependent variable:
# Generate outcome probabilities for each category
p2 <- exp(.2*x1 + .7*x2 - 1.3*x3)/
  (1 + exp(.2*x1 + .7*x2 - 1.3*x3) + exp(-.3*x1 - 1.3*x2 + x3) + 
     exp(0.2*x1 + .6*x2 - x3) + exp(.1*x1 + .3*x2 - .8*x3))
p3 <- exp(-.3*x1 - 1.3*x2 + x3)/
  (1 + exp(.2*x1 + .7*x2 - 1.3*x3) + exp(-.3*x1 - 1.3*x2 + x3) + 
     exp(0.2*x1 + .6*x2 - x3) + exp(.1*x1 + .3*x2 - .8*x3))
p4 <- exp(0.2*x1 + .6*x2 - x3)/
  (1 + exp(.2*x1 + .7*x2 - 1.3*x3) + exp(-.3*x1 - 1.3*x2 + x3) + 
     exp(0.2*x1 + .6*x2 - x3) + exp(.1*x1 + .3*x2 - .8*x3))
p5 <- exp(.1*x1 + .3*x2 - .8*x3)/
  (1 + exp(.2*x1 + .7*x2 - 1.3*x3) + exp(-.3*x1 - 1.3*x2 + x3) + 
     exp(0.2*x1 + .6*x2 - x3) + exp(.1*x1 + .3*x2 - .8*x3))
p1 <- 1 - p2 - p3 - p4 - p5
...
results_per_samp_third <- function(iterations, population, 
  ...
  MRNNI <- function(iterations, samp){
    ...
    # If two or more regression models were applied, regress the dependent 
    # variable on the predicted values and save the new predicted values. 
    temporary <- cbind(boot[boot$response_indicators == 1, "y"], 
                       pred1.1, pred1.2)
    temporary <- as.data.frame(temporary)
...
# Generate dependent variable:
# Generate outcome probabilities for each category
p2 <- exp(-.2*x2^2 - .1*x1*x3 - .2*x1 + .6*x2 + 1.3*x3) /
  (1 + exp(-.2*x2^2 - .1*x1*x3 - .2*x1 + .6*x2 + 1.3*x3) + 
     exp(-.1*x2^2 - .3*x1*x3 + 0.4*x1 + .3*x2 + .6*x3) + 
     exp(.1*x2^2 - .2*x1*x3 - .2*x1 + .4*x2 - x3) +
     exp(.2*x2^2 - .3*x1*x3 + .1*x1 - .6*x2 - 0.8*x3))
p3 <- exp(-.1*x2^2 - .3*x1*x3 + 0.4*x1 + .3*x2 + .6*x3) /
  (1 + exp(-.2*x2^2 - .1*x1*x3 - .2*x1 + .6*x2 + 1.3*x3) + 
     exp(-.1*x2^2 - .3*x1*x3 + 0.4*x1 + .2*x2 + .6*x3) + 
     exp(.1*x2^2 - .2*x1*x3 - .2*x1 + .4*x2 - x3) +
     exp(.2*x2^2 - .3*x1*x3 + .1*x1 - .6*x2 - 0.8*x3))
p4 <- exp(.1*x2^2 - .2*x1*x3 - .2*x1 + .4*x2 - x3) /
  (1 + exp(-.2*x2^2 - .1*x1*x3 - .2*x1 + .6*x2 + 1.3*x3) + 
     exp(-.1*x2^2 - .3*x1*x3 + 0.4*x1 + .3*x2 + .6*x3) + 
     exp(.1*x2^2 - .2*x1*x3 - .2*x1 + .4*x2 - x3) +
     exp(.2*x2^2 - .3*x1*x3 + .1*x1 - .6*x2 - 0.8*x3))
p5 <- exp(.2*x2^2 - .3*x1*x3 + .1*x1 - .6*x2 - 0.8*x3) /
  (1 + exp(-.2*x2^2 - .1*x1*x3 - .2*x1 + .6*x2 + 1.3*x3) + 
     exp(-.1*x2^2 - .3*x1*x3 + 0.4*x1 + .3*x2 + .6*x3) + 
     exp(.1*x2^2 - .2*x1*x3 - .2*x1 + .4*x2 - x3) +
     exp(.2*x2^2 - .3*x1*x3 + .1*x1 - .6*x2 - 0.8*x3))
