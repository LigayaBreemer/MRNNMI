...
standard_error <- function(proportions, M){
  sqrt(1/M * sum(proportions*(1 - proportions)/500) + (1 + 1/M) / (M - 1) * 
...
imputed_aggregate <- sapply(imputed_data, function(x){
...
imputed_aggregate <- rowMeans(imputed_aggregate)
...
imputed_men <- sapply(imputed_dom1, function(x){
  categories <- unique(x[, 7])
...
imputed_men <- rowMeans(imputed_men)
imputed_men <- c(imputed_men, 0)
# Domain 2 
imputed_women <- sapply(imputed_dom2, function(x){
  categories <- unique(x[, 7])
  sapply(categories, FUN = function(y){
    indicator <- x[,7] == y
    weights <- x[indicator, 6]
...
imputed_women <- rowMeans(imputed_women)
